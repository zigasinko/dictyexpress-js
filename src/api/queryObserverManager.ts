...
export const clearObservers = async (): Promise<void> => {
    ...
    try {
        ...
        forwardToSentry('Error unsubscribing from queryobserver.', error);
    }
...
const update = (message: Message, currentItems: unknown[]): unknown[] => {
    ...
    switch (message.msg) {
        ...
        case MESSAGE_REMOVED: {
            items = items.filter(
                (item) =>
                    _.get(item, message.primary_key) !== _.get(updatedItem, message.primary_key),
            );
            break;
        }
        case MESSAGE_CHANGED: {
            const index = items.findIndex(
                (item) =>
                    _.get(updatedItem, message.primary_key) === _.get(item, message.primary_key),
            );
            if (index >= 0) {
                // Remove item at previous index and insert updated one at correct order index.
