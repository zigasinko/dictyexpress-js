...
export const fetchSelectedDifferentialExpressionGenesEpic: Epic<Action, Action, RootState> = (
    action$,
    state$,
) => {
    return action$.pipe(
        ...
        mergeMap(([, state]) => {
            ...
            return from(
                ...
                catchError((error) =>
                    of(handleError(`Error retrieving genes in differential expression. `, error)),
                ),
