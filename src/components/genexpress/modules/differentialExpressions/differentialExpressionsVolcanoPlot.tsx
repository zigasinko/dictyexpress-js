import React, { ReactElement, useRef, useMemo, useCallback, useState, useEffect } from 'react';
...
const DifferentialExpressionsVolcanoPlot: React.FunctionComponent<VolcanoPlotVegaProps> = ({
    data,
    highlightedGenesIds,
    selectedGenesIds,
    logFcOutliersLimit,
    ...
    probField,
    onSelect,
}: VolcanoPlotVegaProps): ReactElement => {
    ...
    useEffect(() => {
        setSelectedGenePoints(_.filter(data, (datum) => selectedGenesIds.includes(datum.geneId)));
    ...
    const getRange = useCallback((): { minX: number; maxX: number; minY: number; maxY: number } => {
    ...
    const updatableDataDefinitions: Array<DataDefinition> = useMemo(
        ...
        [data, highlightedGenePoints, selectedGenePoints, thresholds.fcLog, thresholds.pValueLog],
    );
                ...
                handler: (_name: string, value: any): void => {
                    if (value != null && value.length > 0) {
                        const selectedVolcanoPoints = value.map(
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            (volcanoPointObject: { geneId: string }) => volcanoPointObject.geneId,
                        );
